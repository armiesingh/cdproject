...
directory="/Users/armaansingh/desktop/ABDocking1"
...
receptor="/Users/armaansingh/desktop/ABDocking1/7p2y.pdbqt"
...
for ligand in *.pdbqt; do
    ...
    "$vina_path" --receptor "$receptor" --ligand "$ligand" --out "$output_file" --log "$log_file" --config conf1.txt
